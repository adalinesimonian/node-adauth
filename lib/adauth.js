...
ADAuth.prototype._search = function(searchBase, options, callback) {
  ...
  var bindFunc = options.userClient ? function(cb) {
    cb();
  } : self._adminBind;
...
ADAuth.prototype._findUser = function(username, options, callback) {
  ...
  validateQuery(function(err) {
    ...
    self._search(self.opts.searchBase, validateOpts, function(searchErr, result) {
      if (searchErr) {
        self.log &&
          self.log.trace('AD authenticate: user search error: %s %s %s',
            searchErr.code, searchErr.name, searchErr.message);
        return callback(searchErr);
      }
...
ADAuth.prototype._escapeADString = function(str) {
  var hex; var
    es = str.replace(adUnsafeChars, function(match) {
...
ADAuth.prototype._findObjectBySID = function(sid, options, callback) {
  var self = this;
  var findSID = (typeof (sid) !== 'string' && Array.isArray(sid))
  ...
  var resolve = function(obj, done) {
    ...
    self._search(self.opts.groupSearchBase, searchOpts, function(err, result) {
      ...
      if (obj.primaryGroupID) {
        ...
        self._findObjectBySID(primaryGroupSID, searchOpts,
          function(findErr, primaryGroup) {
            if (findErr) {
              self.log &&
              self.log.trace('find primary group by SID: search error %s %s %s',
                findErr.code, findErr.name, findErr.message);
              return done(findErr);
            }
      ...
      self._getGroups(user, opts, function(groupErr, userWithGroups) {
        ...
        if (self.opts.cache) {
          bcrypt.hash(password, self._salt, function(err, hash) {
            if (err) {
              self.log && self.log.trace('AD authenticate: bcrypt error, not caching %s', err);
            } else {
